...
public class GatewayConfig {
    ...
    public GlobalFilter customGlobalFilter() {
        return (exchange, chain) -> exchange.getPrincipal()
                .map(Principal::getName).defaultIfEmpty(UNAUTHORIZED_USER_NAME)
                .map(principal -> {
                ...
                .flatMap(chain::filter).then(Mono.fromRunnable(() -> {
                    System.out.println("Request from user: " + exchange.getRequest().getHeaders().get(HTTP_HEADER_AUTH_USER_ID)
                            +" request: "+exchange.getRequest().getURI());
                }));
    }
